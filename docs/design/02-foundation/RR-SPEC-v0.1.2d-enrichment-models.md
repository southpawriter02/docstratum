# v0.1.2d — Enrichment Models: Semantic Extension Schema
...
> **Goal:** Define the Layer 2–3 extended schema models that represent what DocStratum adds on top of a parsed llms.txt file — concepts, few-shot examples, LLM instructions, and metadata (`enrichment.py`).
> **Traces to:** FR-002 (v0.0.5a); DECISION-002, -004, -005 (v0.0.4d); v0.0.1b (P0 specification gaps)

---

## Why This Sub-Part Exists

This file represents the **enrichment layer** — the models for content that DocStratum adds to a file beyond what the base spec provides. These models map directly to the three P0 specification gaps identified in v0.0.1b and the 3-Layer Architecture from DECISION-002:

- **Layer 2 (Concept Map):** `Concept`, `ConceptRelationship`, `RelationshipType`
- **Layer 3 (Few-Shot Bank):** `FewShotExample`, `LLMInstruction`
- **File-level Metadata:** `Metadata`

A file that includes these enrichments scores higher at L4 (DocStratum Extended); their absence triggers I001–I003 informational diagnostics but does NOT fail validation.
...
| ID | Decision | How Applied in v0.1.2d |
|----|----------|----------------------|
...
- [ ] All enrichment models (`RelationshipType`, `ConceptRelationship`, `Concept`, `FewShotExample`, `LLMInstruction`, `Metadata`) importable
...
- [ ] `black --check` and `ruff check` pass on enrichment.py
